...
const RidePopUp = (props) => {
  const confirmRide = async () => {
    try {
      console.log("🛠️ Confirming ride:", props.ride?._id);
      ...
      if (response.status === 200) {
        console.log("✅ Ride confirmed, opening OTP popup");
        props.setConfirmRidePopupPanel(true); // Show OTP popup
        props.setRidePopupPanel(false);       // Hide this popup
      }
    } catch (err) {
      console.error("❌ Failed to confirm ride:", err.response?.data || err.message);
      alert("Could not confirm the ride. Please try again.");
    }
  };
